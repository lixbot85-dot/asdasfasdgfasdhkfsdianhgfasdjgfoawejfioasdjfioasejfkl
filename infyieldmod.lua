...
addcmd('unlockkiller',{'unlockk'},function(args, speaker)
local Killer = args[1]
local Success = false
local target = ReplicatedStorage.Assets.Killers:FindFirstChild(Killer).Config
local Module = require(target)
if not Module.PrivateCharacter and Module.PrivateCharacter == true then
Success = true
if target then 
ReplicatedStorage.Modules.Network.RemoteFunction:InvokeServer("PurchaseContent", target) 
else
Success = false
notify("Aviso", "Não existe na pasta!")
end

else

Success = false
notify("Aviso", "Falhado! Personagem é privado!")

end

end)

addcmd('unlocksurvivor',{'unlocks'},function(args, speaker)
local Survival = args[1]
local Success = false
local target = ReplicatedStorage.Assets.Survivors:FindFirstChild(Survival).Config
local Module = require(target)
if not Module.PrivateCharacter and Module.PrivateCharacter == true then
Success = true
if target then 
...
addcmd('unlockemote',{'unlocke'},function(args, speaker)

local Emote = args[1]
local Success = false
local target = ReplicatedStorage.Assets.Emotes:FindFirstChild(Emote)
local Module = require(target)
Success = true
if target then 
ReplicatedStorage.Modules.Network.RemoteFunction:InvokeServer("PurchaseContent", target) 
